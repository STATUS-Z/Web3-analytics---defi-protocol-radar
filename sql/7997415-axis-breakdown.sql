/* Protocol Signal Lab — Axis Breakdown
   Cross-protocol comparison on a single selected metric axis. */
WITH
params AS (
...
ranked AS (
    SELECT z.protocol_name, z.score, p.decomp_axis,
        ROW_NUMBER() OVER (ORDER BY z.score DESC) AS rnk,
        COUNT(*) OVER () AS n
...
leaders AS (
    SELECT
        MAX(CASE WHEN rnk=1 THEN protocol_name END) AS leader,
        MAX(CASE WHEN rnk=n THEN protocol_name END) AS trailer,
        MAX(CASE WHEN rnk=1 THEN score END) AS top_score,
        MAX(CASE WHEN rnk=n THEN score END) AS bot_score,
        MAX(decomp_axis) AS axis_name
    FROM ranked
)
SELECT r.protocol_name AS decomp_protocol, r.score AS decomp_value,
    CONCAT(l.leader, ' leads · ', l.trailer, ' trails') AS leaderboard_note
FROM ranked r CROSS JOIN leaders l
