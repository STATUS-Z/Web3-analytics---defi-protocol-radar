/*
================================================================================
 Protocol Signal Lab — Health Index Timeline (Story zone)
================================================================================
 INSIGHT MODEL (plain language)
   "Health Index" = where TODAY's combined protocol radar strength ranks versus
   the last ~120 days. Scale is always 0–100 so a reader never asks "what is 2.3?"

 MATH (documented for auditors)
   1) Build 8-axis z-scores (0–1) per protocol/day — same radar geometry as Crown.
   2) Shoelace polygon area per protocol/day = relative multi-axis strength.
   3) sector_area(day) = SUM(protocol areas)  — overall DeFi-set strength that day.
   4) health_index = ROUND(100 * PERCENT_RANK() OVER (ORDER BY sector_area), 0)
        → percentile of that day inside the history window (0 = weakest day,
          100 = strongest day in the window).
   5) Bands (fixed, human-readable):
        0–24  Weak   — soft relative to this window
       25–49  Soft   — below typical
       50–74  Strong — healthy / above typical
       75–100 Hot    — top-quartile strength

 CHART ENCODING
   - fill_weak/soft/strong/hot (25 each, stacked) → background band regions
   - health_index on dual-axis line/area → where today sits in those bands
   - No hex/color columns (user sets series colors in Dune UI)
================================================================================
*/
WITH
params AS (
...
sector_daily AS (
    -- Sum of radar polygon areas across the selected protocol set for each day
...
indexed AS (
    SELECT
        date,
        sector_area,
        -- Health Index 0–100: percentile rank of sector_area in the history window
        CAST(ROUND(100.0 * PERCENT_RANK() OVER (ORDER BY sector_area), 0) AS INTEGER) AS health_index,
        -- Stacked 25-point fills = visual band regions (Weak/Soft/Strong/Hot) on left axis
        CAST(25 AS INTEGER) AS fill_weak,
        CAST(25 AS INTEGER) AS fill_soft,
        CAST(25 AS INTEGER) AS fill_strong,
        CAST(25 AS INTEGER) AS fill_hot
    FROM sector_daily
),
banded AS (
    SELECT
        ...
        END AS health_band,
        CASE
            WHEN i.health_index < 25 THEN 'Weak — soft vs this window'
            WHEN i.health_index < 50 THEN 'Soft — below typical'
            WHEN i.health_index < 75 THEN 'Strong — healthy vs typical'
            ELSE 'Hot — top-quartile strength'
        END AS band_insight
    FROM indexed i
)
SELECT
    ...
    health_band,
    band_insight,
    -- Keep raw area for Vault/audit drill (optional; hide in chart)
    ROUND(sector_area, 4) AS sector_area_raw
FROM banded
ORDER BY date
